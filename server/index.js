...
app.use(cors({
  origin: ['https://cultivar-psi.vercel.app', 'http://localhost:3000'],
  credentials: true,
}));
...
// Health check
app.get('/', (req, res) => res.json({ status: 'ok', message: 'Cultivar API running' }));
...
async function runMigrations() {
  try {
    ...
    `);
    await db.query(`CREATE TABLE IF NOT EXISTS batch_events (
      id SERIAL PRIMARY KEY, batch_id INTEGER NOT NULL REFERENCES batches(id) ON DELETE CASCADE,
      type VARCHAR(50), lat DECIMAL, lng DECIMAL, place_name TEXT,
      user_id INTEGER REFERENCES users(id), timestamp TIMESTAMP DEFAULT NOW()
    );`);
    await db.query(`CREATE TABLE IF NOT EXISTS farmer_ratings (
      id SERIAL PRIMARY KEY, farmer_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      consumer_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      batch_id INTEGER NOT NULL REFERENCES batches(id) ON DELETE CASCADE,
      rating INTEGER NOT NULL CHECK (rating >= 1 AND rating <= 5),
      review TEXT NOT NULL, review_photo TEXT, created_at TIMESTAMP DEFAULT NOW(),
    ...
    const cols = [
      ...
      `ALTER TABLE batches ADD COLUMN IF NOT EXISTS farmer_id INTEGER`,
